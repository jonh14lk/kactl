...
struct pt {  // ponto
  ...
  bool operator==(const pt p) const { return eq(x, p.x) and eq(y, p.y) and eq(z, p.z); }
  ...
  friend istream& operator>>(istream& in, pt& p) { return in >> p.x >> p.y >> p.z; }
};
struct line {  // reta
  pt p, q;
  line() {}
...
pt convert(ld rho, ld th, ld phi) { return pt(sin(phi) * cos(th), sin(phi) * sin(th), cos(phi)) * rho; }
...
// distancia
ld dist(pt a, pt b) { return sqrt(sq(a.x - b.x) + sq(a.y - b.y) + sq(a.z - b.z)); }
...
ld sdist(pt p, plane P) { return P.eq[0] * p.x + P.eq[1] * p.y + P.eq[2] * p.z + P.eq[3]; }
